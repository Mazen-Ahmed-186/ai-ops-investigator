...
describe("executeAutomationAction", () => {
  ...
  it("escalates an ambiguous started notification retry without replaying the write", async () => {
    const automationStore = new InMemoryAutomationStore();
    ...
    run = {
      ...run,
      investigationRunId: "INV-NOT-AMBIGUOUS-1",
    };
    ...
    run = {
      ...run,
      remediationRunId: "REM-NOT-AMBIGUOUS-1",
    };
    ...
    await remediationStore.save({
      id: "REM-NOT-AMBIGUOUS-1",

      orderId: "ORD-NOT-1",

      automationRunId: "AUTO-NOT-AMBIGUOUS-1",
    ...
    const executeRetryNotification = vi.fn<
      typeof executeAuditedRetryNotification
    >(async () => {
      throw new Error("Ambiguous execution must not be replayed.");
    });
    ...
    expect(result.executionId).toBe("ACT-NOT-AMBIGUOUS-1");
    ...
    expect(result.run).toMatchObject({
      ...
      actionExecutionId: "ACT-NOT-AMBIGUOUS-1",
    });
